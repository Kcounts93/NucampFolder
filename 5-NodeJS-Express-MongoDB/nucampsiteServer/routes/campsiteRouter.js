...
campsiteRouter
  ...
  .post(authenticate.verifyAdmin(req.user), (req, res, next) => {
  ...
  .delete(authenticate.verifyAdmin(req.user), (req, res, next) => {
    Campsite.deleteMany()
      .then((response) => {
        res.statusCode = 200;
        res.setHeader("Content-Type", "application/json");
...
campsiteRouter
  ...
  .put(authenticate.verifyAdmin(req.user), (req, res, next) => {
